...
double LaserTagWorld_U::noise_sigma_ = DEFAULT_NOISE_SIGMA;

bool LaserTagWorld_U::Connect(){
    ...
    if (client.call(srv))
    {
        ...
        if(action == TAG && srv.response.tag_success == true)
        {
            ...
            for (int dir = 0; dir < NUM_LASER_DIRECTIONS; dir++) {
                LaserTag_U::SetReading(obs, TERMINATION_OBSERVATION, dir);
        ...
        else
        {
            ...
            for (int dir = 0; dir < 8; dir++) {
                LaserTag_U::SetReading(obs, laser_obs[dir], dir);
